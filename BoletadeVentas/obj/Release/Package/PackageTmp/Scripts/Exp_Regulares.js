function goExpRegulares()
{
    var ieversion=/*@cc_on function(){ switch(@_jscript_version){ case 1.0:return 3;
        case 3.0:return 4;case 5.0:return 5; case 5.1:return 5; case 5.5:return 5.5;
        case 5.6:return 6; case 5.7:return 7; }}()||@*/0;
/* Si es Internet Explorer 6 */
          
    if (ieversion != 6) {
        $(".solo-numero").keyup(function () {

            if ($(this).val() != "") {

                $(this).val($(this).attr('value').replace(/[^0-9]/g, ''));
              
            }

        });

        $(".solo-letras").keyup(function(){ 
            if ($(this).val() != '') 
                $(this).val($(this).attr('value').replace(/[^A-Z a-zÑñáéíóúÁÉÍÓÚ,.:-]/g, "")); 
        });

        $(".alfa-num").keyup(function(){ 
            if ($(this).val() != '') 
                $(this).val($(this).attr('value').replace(/[^0-9A-Z a-zÑñáéíóúÁÉÍÓÚ,.:()-]/g, "")); 
        });	

        $(".dv").keyup(function(){ 
            if ($(this).val() != '') 
                $(this).val($(this).attr('value').replace(/[^0-9kK]/g, "")); 
        });

        $(".fono").keyup(function(){ 
            if ($(this).val() != '') 
                $(this).val($(this).attr('value').replace(/[^0-9()-]/g, "")); 
        });	

        $(".rut").keyup(function(){ 
            if ($(this).val() != '') 
                $(this).val($(this).attr('value').replace(/[^0-9-]/g, "")); 
        });
    }
}
   